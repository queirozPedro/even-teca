...
<div class="main-center" style="max-width: 700px; margin: 90px auto 0 auto; background: #fff; border-radius: 18px; box-shadow: 0 4px 24px #0002; padding: 2.5rem 2.5rem 2rem 2.5rem;">
    <h2 class="home-event-title" style="text-align:center; margin-bottom: 2rem;">Pagamento da Inscrição</h2>
    ...
    <form method="POST" action="{{ route('registrations.pay', $event->id) }}" style="max-width:400px; margin:0 auto;">
        @csrf
        <input type="hidden" name="amount" value="{{ $event->price }}">
        <div class="mb-3" style="margin-bottom:1.2rem;">
            <label for="payment_method" class="form-label" style="font-weight:bold;">Método de Pagamento</label>
            <select name="payment_method" id="payment_method" class="form-control" required style="width:100%; padding:0.7rem 1rem; border:1px solid #cbd5e1; border-radius:0.6rem; background:#f8fafc; margin-top:0.2rem; margin-bottom:0.3rem;">
        ...
        <div class="mb-3" style="margin-bottom:1.2rem;">
            <span class="form-label" style="font-weight:bold;">Valor:</span>
            <span style="color:#2563eb; font-weight:bold; margin-left:0.5rem;">R$ {{ number_format($event->price, 2, ',', '.') }}</span>
        </div>
        <div style="display:flex; gap:1rem; align-items:center; justify-content:center;">
            <button type="submit" class="btn btn-success" style="background:#22c55e; color:#fff; border-radius:8px; padding:10px 24px; font-weight:bold; text-decoration:none; border:none;">Pagar</button>
            <a href="{{ route('registrations.show', $event->id) }}" class="btn btn-secondary ml-2" style="background:#e5e7eb; color:#2563eb; border-radius:8px; padding:10px 24px; font-weight:bold; text-decoration:none;">Cancelar</a>
        </div>
    </form>
</div>
@endsection
